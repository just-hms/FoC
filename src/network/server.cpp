...
int Server::Bind() noexcept {
    ...
    auto res = bind(this->listener, (sockaddr *)&address, sizeof(address));
    if (res == -1) {
        return -1;
    }
}

int Server::Bind(int port) noexcept {
    this->port = port;
    return this->Bind();
}
...
void Server::Listen() noexcept {
    ...
    while (!this->stop) {
        ...
        for (int fd = 0; fd <= fdmax; fd++) {
            ...
            auto resp =
                this->router->Handle(fd, std::string(res.begin(), res.end()));
...
int Server::acceptNewConnection(fd_set *master) noexcept {
    ...
    auto newsd =
        accept(this->listener, (sockaddr *)&cl_addr, (socklen_t *)&addrlen);
...
void Server::Stop() noexcept { this->stop = true; }
