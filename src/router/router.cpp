...
Json::Value Router::Login(Context *ctx) {
    ...
    auto [us, err] = this->repo->Login(content["username"].asString(),
                                       content["password"].asString());

    if (err != entity::ERR_OK) {
        return ExitWithJSON(STATUS_UNAUTHORIZED, "wrong username or password");
    }

    // add user to the map of logged in users
    this->users[ctx->connectionID] = us;

    return ExitWithJSON(STATUS_OK);
}

Json::Value Router::Balance(Context *ctx) {
    if (ctx->user == nullptr) {
        return ExitWithJSON(STATUS_UNAUTHORIZED);
    }
    ...
    if (err != entity::ERR_OK) {
        return ExitWithJSON(STATUS_INTERNAL_ERROR, "internal error");
    }
...
Json::Value Router::Transfer(Context *ctx) {
    if (ctx->user == nullptr) {
        return ExitWithJSON(STATUS_UNAUTHORIZED);
    }
    ...
    auto t = entity::Transaction{.from = ctx->user->username,
...
Json::Value Router::History(Context *ctx) {
    if (ctx->user == nullptr) {
        return ExitWithJSON(STATUS_UNAUTHORIZED);
    }
...
void Router::Disconnect(int sd) {
    ...
    std::cout << "/" << route << std::string(10 - route.length(), ' ') << 201
              << std::endl;
    this->users.erase(sd);
}
...
std::string Router::Handle(int sd, std::string message) {
    ...
    try {
        ...
        if (route == "login")
            ...
            out = this->Balance(&ctx);
        else if (route == "transfer")
            out = this->Transfer(&ctx);
        else if (route == "history")
            out = this->History(&ctx);
    } catch (...) {
        out = ExitWithJSON(STATUS_BAD_REQUEST);
    }

    // log the request
    std::cout << "/" << route << std::string(10 - route.size(), ' ')
              << out["status"] << std::endl;
