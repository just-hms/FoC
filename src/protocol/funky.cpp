...
void FunkyProtocol::Disconnect(int sd) { this->sessions.erase(sd); }

std::tuple<FunkySecuritySuite, entity::Error> FunkyProtocol::RightHandshake(
    int sd) {
    ...
    auto asy =
        sec::AsymCrypt(this->dataPath + std::string(this->name + sec::PRIVK),
                       "", this->secret);

    asy.setPeerKey(this->dataPath +
                   std::string(message.begin(), message.end()) + sec::PUBK);
    ...
    signedDH.clear();
    signedDH.insert(signedDH.end(), res.begin(), res.end());
    signedDH.insert(signedDH.end(), encodedRightDH.begin(),
                    encodedRightDH.end());
    ...
    if (err != entity::ERR_OK || res.size() < 512)
    ...
    HSsession.insert(HSsession.end(), encodedRightDH.begin(),
                     encodedRightDH.end());
    HSsession.insert(HSsession.end(), encodedLeftDH.begin(),
                     encodedLeftDH.end());
    ...
    hsMacSignAndMac.insert(hsMacSignAndMac.end(), hsMacSign.begin(),
                           hsMacSign.end());
    ...
    if (err != entity::ERR_OK || recvHsMAC.size() < 64)
        return {FunkySecuritySuite{}, entity::ERR_BROKEN};
    ...
    if (recvHsMAC != hsMac)
        return {FunkySecuritySuite{}, entity::ERR_DURING_HANDSHAKE};

    return {suite, entity::ERR_OK};
}

std::tuple<FunkySecuritySuite, entity::Error> FunkyProtocol::LeftHandshake(
    int sd) {
    FunkySecuritySuite suite;

    auto asy = sec::AsymCrypt(
        this->dataPath + std::string(this->name + sec::PRIVK),
        this->dataPath + std::string(this->peerName + sec::PUBK), this->secret);

    // 1. Send a message with the username
    auto err =
        RawSend(sd, std::vector<uint8_t>(this->name.begin(), this->name.end()));
    if (err != entity::ERR_OK) return {FunkySecuritySuite{}, err};
...
std::tuple<std::string, entity::Error> FunkyProtocol::Receive(int sd) {
    ...
    auto expectedMac =
        std::vector<uint8_t>(res.end() - sec::MAC_LEN, res.end());
    auto encrypted =
        std::vector<uint8_t>(res.begin(), res.end() - sec::MAC_LEN);
    ...
    counter.insert(counter.end(), encrypted.begin(),
                   encrypted.begin() + sec::CTR_LEN);
    ...
    if (counterINT - 1 != secSuite.sym->getCounter(1))
    ...
    return {std::string(res.begin(), res.end()), entity::ERR_OK};
}
