...
// loads users's public key contained in path into a hash map and assigns to
// each user a progressive ID
AsymCrypt::AsymCrypt(std::string privk_file, std::string pubk_file,
                     std::string pwd) {
...
void AsymCrypt::setPeerKey(std::string pubk_file) { this->pubk = pubk_file; }

// encrypts mes using userID's public key
std::tuple<std::vector<uint8_t>, entity::Error> AsymCrypt::sign(
    std::vector<uint8_t> mess) {
    FILE *fp = fopen((this->privk).c_str(), "r");
    if (fp == NULL) {
        std::cerr << "Couldn't open AsymCrypt private key file " << this->privk
                  << std::endl;
    ...
    EVP_PKEY *key =
        PEM_read_PrivateKey(fp, NULL, NULL, (void *)this->privk_pwd.data());
    if (key == NULL) {
...
std::tuple<bool, entity::Error> AsymCrypt::verify(
    std::vector<uint8_t> msg, std::vector<uint8_t> signature) {
    if (this->pubk.size() == 0) {
    ...
    if (fp == NULL) {
        std::cerr << "Couldn't open AsymCrypt public key file "
                  << (this->pubk).c_str() << std::endl;
        return {false, entity::ERR_FILE_NOT_FOUND};
    }
    defer { fclose(fp); };
