...
class AsymCrypt {
   ...
   public:
    AsymCrypt(std::string path_private_key, std::string path_public_key_peer,
              std::string password);
    void setPeerKey(std::string path_public_key_peer);
    std::tuple<std::vector<uint8_t>, entity::Error> sign(
        std::vector<uint8_t> msg);
    std::tuple<bool, entity::Error> verify(std::vector<uint8_t> msg,
...
class SymCrypt {
   ...
   public:
    ...
    std::tuple<std::vector<uint8_t>, entity::Error> encrypt(
        std::vector<uint8_t> plaintext);
    std::tuple<std::vector<uint8_t>, entity::Error> decrypt(
...
class Hmac {
   ...
   public:
    ...
    std::tuple<std::vector<uint8_t>, entity::Error> MAC(
...
std::tuple<std::string, entity::Error> HashAndSalt(std::string password,
                                                   std::string salt = "");
bool VerifyHash(std::string hashAndSalt, std::string password);

entity::Error generateRSAkeys(std::string path, std::string password,
                              unsigned int bits);
std::tuple<std::vector<uint8_t>, entity::Error> encodePeerKey(
    EVP_PKEY *keyToEncode);
std::tuple<EVP_PKEY *, entity::Error> decodePeerKey(
...
std::tuple<EVP_PKEY *, entity::Error> retrieveDHparam(
    std::vector<uint8_t> DHserialized);
std::tuple<std::vector<uint8_t>, entity::Error> derivateDH(
    EVP_PKEY *your_dhkey, EVP_PKEY *peer_dhkey);
std::tuple<std::vector<uint8_t>, entity::Error> keyFromSecret(
