...
unsigned int SymCrypt::getCounter(int index) { return this->counters[index]; }

// encrypts pt by using the userID's session key
std::tuple<std::vector<uint8_t>, entity::Error> SymCrypt::encrypt(
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, this->key,
                           iv.data()) <= 0) {
...
std::tuple<std::vector<uint8_t>, entity::Error> SymCrypt::decrypt(
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, this->key,
                           iv.data()) <= 0) {
        std::cerr << "Unable to initialize a context for SymCrypt" << std::endl;
