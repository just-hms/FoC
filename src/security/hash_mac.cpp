...
// given a buffer and its length, returns a hex string of the contents of such
// buffer
...
Hmac::Hmac() { RAND_bytes(this->key, sizeof(this->key)); }

// initialize a Hmac object with specified key, if the key is too short then
// it's increased in size
...
std::tuple<std::vector<uint8_t>, entity::Error> Hmac::MAC(
...
std::tuple<std::string, entity::Error> Hash(std::string data) {
    ...
    if (EVP_DigestUpdate(ctx, (unsigned char *)data.c_str(), data.size()) <=
        0) {
...
bool VerifyHash(std::string hashandsalt, std::string password) {
    auto salt =
        hashandsalt.substr(hashandsalt.find("|") + 1, hashandsalt.size());
    auto [computedHash, err] = HashAndSalt(password, salt);

    if (err != entity::ERR_OK) return false;

    return CRYPTO_memcmp((void *)&computedHash[0], (void *)&hashandsalt[0],
